...
class LogicService {
  ...
  async newSavedConfig(name = null, interval, maxDepth, timeout, callback) {
    try {
      //let dbName = 'test';
      //let savedConfigDao = new SavedConfigurarionDao(dbName);
      ...
      savedConfigDao.insertValue(savedConfigEntity, (err, newEntity) => {
        if (err != null) {
          callback(err, null);
        } else {
          callback(null, newEntity.getID());
        }
      });
    } catch (error) {
      throw error;
