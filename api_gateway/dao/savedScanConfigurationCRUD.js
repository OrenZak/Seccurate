...
class SavedConfigurationCRUD {
  constructor(db) {
    //should become db_type and read from globals
  ...
  updateValue(new_value, callback) {
    ...
    console.log('object to update: ', new_value, new_value.getName());
    ...
    this.conn.query(
      ...
      [
        this.table_name,
        new_value.getName(),
        new_value.getMaxDepth(),
        new_value.getTimeout(),
        new_value.getInterval(),
        new_value.getID()
      ],
      err => {
        if (err) {
  ...
  getAll(callback, page = 0, size = 10) {
    ...
    this.conn.query(
      sql,
      [this.table_name, intSize, intPage * intSize],
