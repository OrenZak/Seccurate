...
class LogicService():
    ...
    def startScan(self, pageEntity=None, sessionEntity=None):#, db_type=None, vuln_table_name=None):  # start scan by using the client info data
        ...
        if self.__scanType == "ALL":
            self.__scanForRXSS(pageEntity=pageEntity, forms=forms, links=links)#sessionEntity=sessionEntity)
            self.__scanForSqlInjection(pageEntity=pageEntity, forms=forms, links=links)#sessionEntity=sessionEntity)
