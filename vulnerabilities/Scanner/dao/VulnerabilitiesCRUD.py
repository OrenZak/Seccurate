...
def createVulnerability(vuln, table_name):
    """
...
def getVulns(table_name, size=10, page=0):
    """
...
def deleteVulnByID(id, table_name):
    if getVulnByID(id, table_name) is None:
        raise Exception("no such vulnerability")
    with sqlite3.connect(config.get('VulnServiceDB', "test")) as db:
        cursor = db.cursor()
...
def deleteAllDataFromTable(table_name):
    with sqlite3.connect(config.get('VulnServiceDB', "test")) as db:
        cursor = db.cursor()
